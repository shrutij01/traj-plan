function [energy_STOMP, opt_traj] = STOMP
%% Synthetic Data
clc 
% Parameters
...
%     [T,R,R_inv,M,eps_t] = temporal_constraint(T,N,A); %STOMP
...
% 3) finding the change in each waypoint for STOMP
    ...
    delta_theta_x = M*delta_theta_x(2:N);
    delta_theta_y = M*delta_theta_y(2:N);
    delta_theta_x([1,N]) = 0;
    delta_theta_y([1,N]) = 0; 
...
E_c = zeros(N,1);
for i = 1:N-1
    E_c(i) =  0.01*(c_d *norm(([opt_traj(i,1);opt_traj(i,2)] - [opt_traj(i+1,1);opt_traj(i+1,2)])/tm_init - [ocean_x(i);ocean_y(i)])^3)*tm_init;
end
energy_STOMP = sum(E_c);
...
% disp(temp1)
% temp1 = ['Energy cost of the opitmal trajectory is : ',num2str(energy_cost)];
% disp(temp1)
% 
end
