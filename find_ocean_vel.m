function [vel_ocean] = find_ocean_vel(theta_x,theta_y,U,V,X_loc,Y_loc)
...
for i = 1:N
    for j = 1:K
        ...
        elseif (length(x_limit)==2 && length(y_limit)==1)           
            vel_u(i,j) = 0.5*(U(x_limit(1),y_limit(1)) + U(x_limit(2),y_limit(1))) ;
            vel_v(i,j) = 0.5*(V(x_limit(1),y_limit(1)) + V(x_limit(2),y_limit(1))) ;
